    ...
    try:
        ...
        assert response.json() == {
            ...
            "checks": {
                ...
                "anthropic_compat": {"status": "ok", "detail": None},
...
def test_health_anthropic_compat_failure(monkeypatch) -> None:
    def fake_db_check(_settings) -> None:
        return None

    def fake_anthropic_compat_check(_settings) -> None:
        raise RuntimeError("chat model missing")
    ...
    try:
        ...
        assert response.json() == {
            ...
            "checks": {
                "app": {"status": "ok", "detail": None},
                "db": {"status": "ok", "detail": None},
                "anthropic_compat": {"status": "error", "detail": "chat model missing"},
                "ollama_native": {"status": "ok", "detail": None},
            },
        }
    finally:
